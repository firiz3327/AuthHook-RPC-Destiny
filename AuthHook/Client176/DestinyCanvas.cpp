...
bool hookCFontDrawTextA() {
    ...
    CFont__DrawTextA_t Hook = [](void* ecx, void* edx, void* pCanvas, int x, int y, unsigned __int16* sText, int nAlpha, int nTabOrg) -> void {
        ...
        if (sText) {
            ...
            /*
            size_t len = wcstombs(nullptr, currentDrawText, 0);
            if (len != (size_t)-1)
            {
                char* pdst = new char[len + 1];
                memset(pdst, 0, (len + 1));
                wcstombs(pdst, currentDrawText, (len + 1));
                for (size_t i = 0; i <= len; i++)
                {
                    printf("%02X ", *((unsigned char*)pdst + i));
                }
                delete[] pdst;
            }
            Log("");
            */
            ...
            /*
            std::wstring ws(currentDrawText);
            std::string str(ws.begin(), ws.end());
            std::string parsed = multi_to_utf8_cppapi(str);

            std::wstring pws = std::wstring(parsed.begin(), parsed.end());
            currentDrawText = pws.c_str();
            */
            ...
            /*
            wprintf(L"%s\n", currentDrawText);
            if (wcscmp(currentDrawText, L"admin") == 0) {
                printf("OOOO");
                currentDrawText = L"あいうえお ０１２３";
            }
            */

            //wprintf(currentDrawText);
            //printf("\n");
        }
...
bool hookTextOutA()
{
    static auto _CreateFontA = decltype(&TextOutA)(GetFuncAddress("GDI32", "TextOutA"));
